...
describe('dashboard contract', () => {
  test('bounds and detaches dashboard queries', () => {
    ...
    result.filters![0]!.field = 'changed';
  ...
  test('detaches a valid snapshot', () => {
    ...
    result.widgets[0]!.metrics[0]!.label = 'Changed';
    expect(snapshot.widgets[0]!.metrics[0]!.label).toBe('Total');
  });

  test.each([
    { version: 0 },
    { widgets: [{ ...snapshot.widgets[0], metrics: [{ ...snapshot.widgets[0]!.metrics[0], href: 'javascript:alert(1)' }] }, snapshot.widgets[1]] },
    { widgets: [{ ...snapshot.widgets[0], metrics: snapshot.widgets[0]!.metrics }, { ...snapshot.widgets[1], id: 'revenue' }] },
