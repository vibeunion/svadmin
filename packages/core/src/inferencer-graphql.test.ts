...
describe('inferFromGraphQL Introspection',() => {
  ...
  test('pluralizes entity names correctly',() => {
    ...
    expect(requireValue(post).label).toBe('Post');
  });

  test('detects required fields from NON_NULL',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const titleField=requireValue(post.fields.find(f => f.key==='title'));
    const contentField=requireValue(post.fields.find(f => f.key==='content'));
  ...
  test('maps Int to number type',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const viewsField=requireValue(post.fields.find(f => f.key==='views'));
  ...
  test('maps Boolean to boolean type',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const field=requireValue(post.fields.find(f => f.key==='is_featured'));
  ...
  test('maps DateTime custom scalar to date',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const field=requireValue(post.fields.find(f => f.key==='created_at'));
  ...
  test('maps JSON custom scalar to json',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const field=requireValue(post.fields.find(f => f.key==='meta'));
  ...
  test('maps Enum to select with options',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const field=requireValue(post.fields.find(f => f.key==='status'));
  ...
  test('maps object relation field to relation',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const authorField=requireValue(post.fields.find(f => f.key==='author'));
  ...
  test('maps author_id relation field to relation from name suffix',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const field=requireValue(post.fields.find(f => f.key==='author_id'));
  ...
  test('maps string array to tags and photo array to images',() => {
    const resources=inferFromGraphQL(sampleGraphQLIntrospection);
    const post=requireValue(resources.find(r => r.name==='posts'));
    const tagsField=requireValue(post.fields.find(f => f.key==='tags'));
    const photosField=requireValue(post.fields.find(f => f.key==='photos'));
  ...
  test('detects CRUD capabilities from root Query and Mutation fields',() => {
    ...
    const user=requireValue(resources.find(r => r.name==='users'));
  ...
  test('include and exclude filters work',() => {
    ...
    expect(requireValue(included[0]).name).toBe('posts');

    const excluded=inferFromGraphQL(sampleGraphQLIntrospection,{ exclude: ['Post'] });
    expect(excluded.length).toBe(1);
    expect(requireValue(excluded[0]).name).toBe('users');
  });
...
describe('inferFromGraphQL SDL String',() => {
  ...
  test('parses SDL string correctly',() => {
    ...
    const account=requireValue(resources[0]);
    expect(account.name).toBe('accounts');
    expect(account.label).toBe('Account');

    const emailField=requireValue(account.fields.find(f => f.key==='email'));
    expect(emailField.type).toBe('email');
    expect(emailField.required).toBe(true);

    const roleField=requireValue(account.fields.find(f => f.key==='role'));
    expect(roleField.type).toBe('select');
    expect(roleField.options?.length).toBe(3);

    const bioField=requireValue(account.fields.find(f => f.key==='bio'));
    expect(bioField.type).toBe('textarea');

    const websiteField=requireValue(account.fields.find(f => f.key==='website'));
