  ...
  const items=$derived.by<BreadcrumbItem[]>(() => {
    ...
    try {
      ...
      while(current) {
        ...
        chain.unshift({ label: current.label||current.name,link: `/${current.name}`,...definedOptions({ icon: current.icon }) });
...
export function useRelation(options: UseRelationOptions) {
  return useList({
    ...options
  });
}
...
export function useNotification() {
  ...
  function sendToast(
    ...
    if(provider) {
      provider.open({ type,message,...definedOptions({ key: options?.key }) });
      return;
    }
...
export function useRadioGroup(options: UseRadioGroupOptions) {
  ...
  // useSelect defaultValue is an array; pass the scalar value through so remote options can restore the selection.
