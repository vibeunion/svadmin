<script lang="ts">
  ...
  const branch = useMessageBranch('MessageBranchNext');

  function next(event: MouseEvent): void {
    onclick?.(event);
    if (!event.defaultPrevented) branch.goToNext();
  }
</script>

<button {...rest} {type} class={cn('svadmin-ai-message-branch-button', className)} data-slot="message-branch-next" aria-label="Next branch" disabled={disabled || branch.totalBranches <= 1} onclick={next}>
  {#if children}{@render children()}{:else}<ChevronRight size={14} aria-hidden="true" />{/if}
</button>
