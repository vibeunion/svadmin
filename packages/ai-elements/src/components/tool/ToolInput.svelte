...
{#if resolvedInput !== undefined}
  <div {...rest} class={cn('svadmin-ai-tool-input', className)} data-slot="tool-input">
    <h4>Parameters</h4>
    <pre use:focusScrollableContent role="region" aria-label="Tool parameters">{formatToolValue(resolvedInput)}</pre>
  </div>
{/if}

<style>
  .svadmin-ai-tool-input { display: grid; grid-template-columns: minmax(0, 1fr); min-width: 0; max-width: 100%; gap: 0.5rem; }
