...
<summary {...rest} class={cn('svadmin-ai-tool-header', className)} data-slot="tool-header">
  ...
  {:else}
    <span class="svadmin-ai-tool-header__summary">
      ...
      <strong title={derivedName}>{derivedName}</strong>
...
<style>
  .svadmin-ai-tool-header { display: flex; box-sizing: border-box; width: 100%; min-width: 0; align-items: center; justify-content: space-between; gap: 1rem; padding: 0.75rem; cursor: pointer; list-style: none; }
  .svadmin-ai-tool-header::-webkit-details-marker { display: none; }
  .svadmin-ai-tool-header__summary { display: flex; flex: 1; align-items: center; gap: 0.5rem; min-width: 0; }
  .svadmin-ai-tool-header__summary strong { flex: 1; min-width: 0; overflow: hidden; font-size: 0.875rem; font-weight: 600; text-overflow: ellipsis; white-space: nowrap; }
  .svadmin-ai-tool-header__summary > :global(svg) { flex: none; color: var(--muted-foreground, currentColor); }
