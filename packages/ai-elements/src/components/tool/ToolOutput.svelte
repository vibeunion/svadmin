...
<script lang="ts">
  ...
  import { focusScrollableContent } from './content-polish-scroll-region.js';
...
{#if resolvedOutput !== undefined || resolvedError}
  <div {...rest} class={cn('svadmin-ai-tool-output', className)} data-slot="tool-output" data-error={resolvedError ? 'true' : undefined}>
    ...
    <pre use:focusScrollableContent role="region" aria-label={resolvedError ? 'Tool error' : 'Tool result'}>{resolvedError ?? formatToolValue(resolvedOutput)}</pre>
  </div>
{/if}

<style>
  .svadmin-ai-tool-output { display: grid; grid-template-columns: minmax(0, 1fr); min-width: 0; max-width: 100%; gap: 0.5rem; }
  ...
  pre:focus-visible { outline: 2px solid var(--ring, currentColor); outline-offset: 2px; }
</style>
