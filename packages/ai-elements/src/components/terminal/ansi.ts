...
function resetStyle(style: AnsiStyle): void {
  for (const key of Object.keys(style)) Reflect.deleteProperty(style, key);
}
...
function setColor(style: AnsiStyle, target: 'foreground' | 'background', code: number): void {
  ...
  const color = ANSI_COLORS[code - offset];
  if (color !== undefined) style[target] = color;
}
...
function applyExtendedColor(style: AnsiStyle, target: 'foreground' | 'background', codes: number[], index: number): number {
  if (codes[index + 1] === 5) {
    ...
    const color = colorIndex === undefined ? undefined : ANSI_COLORS[colorIndex];
    if (color !== undefined) style[target] = color;
    return index + 2;
  }
