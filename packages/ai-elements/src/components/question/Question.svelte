<script lang="ts">
  ...
  async function handleSubmit(event: SubmitEvent): Promise<void> { event.preventDefault(); if (disabled || busy) return; const text = value.text.trim(); if (value.selectedValues.length === 0 && text.length === 0) return; busy = true; try { await onsubmit?.(definedOptions({ selectedValues: value.selectedValues, text: text || undefined }), event); } finally { busy = false; } }
