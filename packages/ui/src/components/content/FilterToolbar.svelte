...
<div class={'flex flex-col gap-2 sm:flex-row sm:items-center ' + className}>
  <div class="relative min-w-0 flex-1">
    ...
    {#if query}<Button variant="ghost" size="icon-xs" class="absolute right-1 top-1/2 -translate-y-1/2" aria-label={clearLabel} title={clearLabel} onclick={() => query = ''}><X class="size-3.5" /></Button>{/if}
  </div>
