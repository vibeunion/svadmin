...
describe('PM workflow acceptance', () => {
  ...
  it('keeps detail visible and isolates deletion behind the action menu and confirmation', async () => {
    ...
    expect((await view.findAllByRole('button', { name: /^Detail$/ })).length).toBeGreaterThan(0);
    expect(view.queryByRole('button', { name: /^Delete$/ })).toBeNull();
    await fireEvent.click(first(await view.findAllByRole('button', { name: 'More actions' })));
    await fireEvent.click(await view.findByRole('menuitem', { name: /^Delete$/ }));
  ...
  it('saves quick edits without changing the route, filters, or selection', async () => {
    ...
    await view.findAllByText('user@example.com');
    const checkbox = first(await view.findAllByRole('checkbox', { name: /user-1/ }));
  ...
  it('groups detail metadata and provides a working ID copy action', async () => {
    ...
    await fireEvent.click(first(await view.findAllByRole('button', { name: /^Detail$/ })));
  ...
  it('offers recovery in both empty list layouts when a search has no results', async () => {
    ...
    await fireEvent.click(last(view.getAllByRole('button', { name: 'Clear all' })));
