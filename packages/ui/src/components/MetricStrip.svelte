 <script lang="ts">
   ...
   const badgeToneMap: Record<NonNullable<NonNullable<MetricStripItem['badge']>['tone']>, string> = {
 ...
 <div
   ...
   aria-label="业务指标概览"
   class={cn(
     'grid divide-y sm:divide-y-0 divide-border overflow-hidden rounded-lg border border-border bg-card shadow-xs',
   ...
   {:else}
     {#each items as item, index (item.id || index)}
       ...
       <svelte:element
         ...
         href={item.href}
         class={cn(
           'group relative flex flex-col justify-between gap-1 p-3.5 sm:p-4 text-left transition-colors duration-150',
           'border-border sm:border-l first:border-l-0',
           isInteractive && 'hover:bg-muted/40 cursor-pointer text-inherit no-underline',
           item.class
         )}
       >
        <div class="flex items-center justify-between gap-2">
          <span class="text-xs font-semibold text-muted-foreground truncate">{item.label}</span>
          {#if item.icon}
