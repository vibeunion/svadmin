...
describe('catalog-derived generation contract', () => {
  test('exports descriptions, required props and validated examples without executable metadata', () => {
    ...
    expect(manifest.widgets[0].propsSchema).toEqual(JSON.parse(JSON.stringify(metricPropsSchema)));
    expect(manifest.widgets[0].description).toContain('scalar');
    expect(manifest.widgets[0].examples).toEqual([{ label: 'Total', format: 'number' }]);
  ...
  test('shares the actual widget schema and narrows resource/field/page policies', () => {
    ...
    expect(compiled.Check({ ...candidate, widgets: [{ ...candidate.widgets[0], props: { label: 'Total', format: 'currency' } }] })).toBe(false);
    expect(compiled.Check({ ...candidate, dataSources: [{ ...candidate.dataSources[0], pageSize: 6 }] })).toBe(false);
    expect(compiled.Check({ ...candidate, dataSources: [{ id: 'orders-data', type: 'resource-list', resource: 'secrets', pageSize: 5 }] })).toBe(false);
    expect(compiled.Check({ ...candidate, dataSources: [{ ...candidate.dataSources[0], sorters: [{ field: 'secret', order: 'asc' }] }] })).toBe(false);
    expect(compiled.Check({ ...candidate, dataSources: [{ ...candidate.dataSources[0], filters: [{ field: 'secret', operator: 'eq', value: 'x' }] }] })).toBe(false);
  });
  ...
  test('task-scoped catalogs constrain both generation and runtime without losing renderer metadata', () => {
    ...
    expect(selected.widgets[0].component).toBe('trusted-renderer');
  ...
  test('rejects duplicate catalog entries, loose object schemas and invalid examples', () => {
    expect(() => createSurfaceCatalogManifest({ ...catalog, widgets: [catalog.widgets[0], catalog.widgets[0]] })).toThrow('duplicate');
    expect(() => createSurfaceCatalogManifest({ ...catalog, widgets: [{ ...catalog.widgets[1], propsSchema: Type.Object({ text: Type.String() }) }] })).toThrow('closed');
    expect(() => createSurfaceCatalogManifest({ ...catalog, widgets: [{ ...catalog.widgets[0], examples: [{ label: 'Total', format: 'currency' }] }] })).toThrow('example');
  });
