...
function providerFixture(overrides: Partial<DataProvider>): DataProvider {
  return {
    getList: unexpected,
    getOne: unexpected,
    create: unexpected,
    update: unexpected,
    deleteOne: unexpected,
    getApiUrl: unexpected,
    ...overrides,
  };
}

function eventFixture(input: Omit<Partial<RequestEvent>, 'cookies'> & {
  cookies?: Partial<RequestEvent['cookies']>;
}): RequestEvent {
  const url = input.url ?? new URL(input.request?.url ?? 'https://admin.example/');
  return {
    url,
    request: input.request ?? new Request(url),
    fetch: unexpected,
    getClientAddress: unexpected,
    locals: {},
    params: {},
    platform: undefined,
    route: { id: null },
    setHeaders: unexpected,
    isDataRequest: false,
    isSubRequest: false,
    isRemoteRequest: false,
    get tracing(): RequestEvent['tracing'] { return unexpected(); },
    ...input,
    cookies: {
      get: unexpected,
      getAll: unexpected,
...
function fileNames(value: unknown): string[] {
  if (!Array.isArray(value)) throw new Error('Expected uploaded files');
  return value.map((entry: unknown) => {
    if (!(entry instanceof File)) throw new Error('Expected an uploaded File');
    return entry.name;
  });
}

function records(value: unknown): Record<string, unknown>[] {
  if (!Array.isArray(value)) throw new Error('Expected record rows');
...
describe('createListLoader search compatibility', () => {
  test('searches every searchable field with one logical OR filter', async () => {
    ...
    const provider = providerFixture({ getList });
...
describe('createAuthGuard AuthProvider compatibility', () => {
  ...
  test('trusts AuthProvider.check without requiring a Lite-only marker cookie', async () => {
    ...
    const response = await createAuthGuard(authProvider(check))({ event: eventFixture(event), resolve });
  ...
  test('allows exported public auth pages and honors a safe provider redirect', async () => {
    ...
    expect(await (await guard({ event: eventFixture(publicEvent), resolve })).text()).toBe('public content');
    expect(check).not.toHaveBeenCalled();

    const protectedEvent = guardEvent('/lite/posts');
    const response = await guard({ event: eventFixture(protectedEvent), resolve });
  ...
  test('keeps root auth routes protected when a custom login page uses another name', async () => {
    ...
    const response = await guard({ event: eventFixture(rootRegisterEvent), resolve });
    expect(response.status).toBe(302);
    expect(response.headers.get('location')).toBe('/auth/sign-in');
    expect(check).toHaveBeenCalledTimes(1);

    const siblingRegisterEvent = guardEvent('/auth/register');
    expect(await (await guard({ event: eventFixture(siblingRegisterEvent), resolve })).text())
      .toBe('public content');
    expect(check).toHaveBeenCalledTimes(1);
  });
});
...
describe('createAuthActions exported page compatibility', () => {
  ...
  test('validates confirmed passwords and delegates optional provider methods', async () => {
    ...
    await expect(actions.register(eventFixture(authRequest('/lite/register', {
      email: 'user@example.com',
      password: 'secret1',
      confirmPassword: 'different',
    })))).resolves.toEqual({ success: false, error: 'Passwords do not match' });
    expect(register).not.toHaveBeenCalled();

    await expect(actions.forgot_password(eventFixture(authRequest('/lite/forgot-password', {
      email: 'user@example.com',
    })))).resolves.toEqual({ success: true });
    await expect(actions.update_password(eventFixture(authRequest('/lite/update-password', {
      password: 'secret1',
      confirmPassword: 'secret1',
    })))).resolves.toEqual({ success: true });
    await expect(actions.update_profile(eventFixture(authRequest('/lite/profile', {
      name: 'Alice',
  ...
  test('preserves the AuthProvider receiver for class-style methods', async () => {
    ...
    } satisfies AuthProvider & { marker: string };

    await expect(createAuthActions(provider).register(eventFixture(authRequest('/lite/register', {
      email: 'user@example.com',
      password: 'secret1',
      confirmPassword: 'secret1',
  ...
  test('does not expose unexpected logout exception details', async () => {
    ...
    await expect(createAuthActions(provider).logout(eventFixture({
      cookies: { delete: deleteCookie },
    }))).resolves.toEqual({ success: false, error: 'Logout failed' });
  ...
  test('does not expose unexpected registration exception details', async () => {
    ...
    await expect(createAuthActions(provider).register(eventFixture(authRequest('/lite/register', {
      ...
      password: 'secret1',
      confirmPassword: 'secret1',
    })))).resolves.toEqual({ success: false, error: 'Registration failed' });
  });
  ...
  test('does not expose unexpected login exception details', async () => {
    ...
    await expect(createAuthActions(provider).login(eventFixture(authRequest('/lite/login', {
      email: 'user@example.com',
      password: 'secret1',
  ...
  test('honors logout action failures and successful provider redirects', async () => {
    ...
    await expect(createAuthActions(failedProvider).logout(eventFixture({
      cookies: { delete: deleteCookie },
    }))).resolves.toEqual({ success: false, error: 'Session could not be revoked' });

    const redirectedProvider = {
      ...failedProvider,
      logout: mock(async () => ({ success: true, redirectTo: '/signed-out' })),
    } satisfies AuthProvider;
    await expect(createAuthActions(redirectedProvider).logout(eventFixture({
      cookies: { delete: deleteCookie },
    }))).rejects.toMatchObject({ status: 303, location: '/signed-out' });
...
describe('createCrudActions array form parsing', () => {
  test('rejects invalid top-level boolean tokens before calling the provider', async () => {
    ...
    const provider = providerFixture({ create });
    ...
    const result = await createCrudActions(provider, booleanResource).create(eventFixture({
  ...
  test('rejects invalid nested boolean tokens before calling the provider', async () => {
    ...
    const result = await createCrudActions(provider, nestedBooleanResource).create(eventFixture({
  ...
  test('does not expose unexpected provider exception details', async () => {
    ...
    const provider = providerFixture({ create, update, deleteOne });
    ...
    await expect(actions.create(eventFixture({
    ...
    await expect(actions.update(eventFixture({
    ...
    await expect(actions.delete(eventFixture({
  ...
  test('enforces ResourceDefinition CRUD flags before invoking the provider', async () => {
    ...
    await expect(createCrudActions(provider, lockedResource).create(eventFixture({
      request: new Request('https://admin.example/lite/locked', { method: 'POST', body: createForm }),
    }))).resolves.toEqual({ success: false, error: 'Create is disabled for this resource' });
    await expect(createCrudActions(provider, lockedResource).update(eventFixture({
      request: new Request('https://admin.example/lite/locked', { method: 'POST', body: updateForm }),
    }))).resolves.toEqual({ success: false, error: 'Edit is disabled for this resource' });
    await expect(createCrudActions(provider, lockedResource).delete(eventFixture({
      request: new Request('https://admin.example/lite/locked', { method: 'POST', body: deleteForm }),
    }))).resolves.toEqual({ success: false, error: 'Delete is disabled for this resource' });
  ...
  test('restores numeric select values before calling create', async () => {
    const create = mock(async ({ variables }: { variables: unknown }) => ({
      data: { id: 1, ...record(variables) },
    }));
    const provider = providerFixture({ create });
    ...
    await createCrudActions(provider, optionResource).create(eventFixture({
  ...
  test('parses sparse bracketed rows, coerces sub-fields, and removes checked rows', async () => {
    ...
    const result = await createCrudActions(provider, resource).create(eventFixture({ request }));
  ...
  test('does not call create when a required array draft is empty', async () => {
    ...
    const result = await createCrudActions(provider, requiredResource).create(eventFixture({ request }));
  ...
  test('does not call update when a real array row omits a required child', async () => {
    ...
    const provider = providerFixture({ update });
    const formData = new FormData();
    ...
    const result = await createCrudActions(provider, resource).update(eventFixture({ request }));
  ...
  test('validates numbers and preserves all uploaded files before calling create', async () => {
    ...
    const result = await createCrudActions(provider, uploadResource).create(eventFixture({ request }));

    expect(result).toEqual({ success: true, id: 1 });
    expect(create).toHaveBeenCalledTimes(1);
    const variables = record(create.mock.calls[0]?.[0].variables);
    ...
    expect(fileNames(variables?.['gallery'])).toEqual(['first.png', 'second.png']);
  });

  test('keeps an existing required upload when edit submits no replacement file', async () => {
    const update = mock(async ({ variables }: { variables: unknown }) => ({
      data: { id: 'upload-1', ...record(variables) },
    }));
    const provider = providerFixture({ update });
    const uploadResource: ResourceDefinition = {
      name: 'uploads',
    ...
    const result = await createCrudActions(provider, uploadResource).update(eventFixture({ request }));
  ...
  test('retains nested edit upload references and lets new Files replace retained references', async () => {
    const update = mock(async ({ variables }: { variables: unknown }) => ({
      data: { id: 'account-1', ...record(variables) },
    }));
    const provider = providerFixture({ update });
    ...
    const result = await createCrudActions(provider, documentsResource).update(eventFixture({ request }));

    expect(result).toEqual({ success: true });
    expect(update).toHaveBeenCalledTimes(1);
    const documents = records(record(update.mock.calls[0]?.[0].variables)['documents']);
    ...
    const attachment = documents[1]?.['attachment'];
    if (!(attachment instanceof File)) throw new Error('Expected a replacement file');
    expect(attachment.name).toBe('replacement.pdf');
    expect(fileNames(documents[1]?.['gallery'])).toEqual(['new-one.png', 'new-two.png']);
  });

  test('leaves delete action validation behavior unchanged', async () => {
    const deleteOne = mock(async () => ({ data: { id: 'account-1' } }));
    const provider = providerFixture({ deleteOne });
    const formData = new FormData();
    ...
    const result = await createCrudActions(provider, resource).delete(eventFixture({ request }));
  ...
  test('only follows same-origin absolute paths after delete', async () => {
    ...
    await expect(
      createCrudActions(provider, resource).delete(eventFixture({ request: externalRequest })),
    ).resolves.toEqual({ success: true });
    ...
    await expect(
      createCrudActions(provider, resource).delete(eventFixture({ request: controlCharacterRequest })),
    ).resolves.toEqual({ success: true });
    ...
    await expect(
      createCrudActions(provider, resource).delete(eventFixture({ request: localRequest })),
  ...
  test('rejects update and delete submissions without a record id', async () => {
    const update = mock(async () => ({ data: { id: 'unexpected' } }));
    const deleteOne = mock(async () => ({ data: { id: 'unexpected' } }));
    const provider = providerFixture({ update, deleteOne });
    ...
    await expect(
      createCrudActions(provider, resource).update(eventFixture({ request: updateRequest })),
    ).resolves.toEqual({ success: false, error: 'Missing record id' });
    await expect(
      createCrudActions(provider, resource).delete(eventFixture({ request: deleteRequest })),
    ).resolves.toEqual({ success: false, error: 'Missing record id' });
    expect(update).not.toHaveBeenCalled();
    expect(deleteOne).not.toHaveBeenCalled();
  });
