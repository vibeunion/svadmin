...
本批只在现有迁移分支实施；不合并、不发布、不部署，不修改数据权限或 Surface v1。
...
本地运行 38 项测试通过（Node 22.16.0，PostCSS，已安装 TypeScript 解析器）：
...
覆盖：变体契约、真实原生 CSS 打包、重复构建、错误导入、层级、fallback
退役范围、锁文件传递依赖、别名与负例。`git diff --check` 通过。

本地没有 Bun、Panda/Svelte 工具链或浏览器，**未在本地运行** Panda 生成、
Svelte 构建/类型检查、发布包隔离消费或新截图对比。新增生成产物断言在
`bun run --cwd packages/ui test:css` 中执行，不能以配置单测代替它们。

当前提交须以 CI 的实际 source SHA、原始日志和截图结果验收。此前 artifact
10574653456 的源码只作为本次检查/打补丁的起点，不证明此次修改通过构建。
