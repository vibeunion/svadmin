...
| CLI `add provider` 支持 17 个官方 data provider（含 dependencyPacks / compat 映射） | 已落地 |
| 新增 `@svadmin/rest`（桥接 `@refinedev/rest`）官方 provider | 已落地 |
...
其他官方 provider 通过 `svadmin add provider <name>` 接入（`rest`、`airtable`、`appwrite`、
