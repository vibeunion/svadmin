<script lang="ts">
  ...
  let { resourceName = 'case_workspace' }: { resourceName?: ResourcesForFeature<'case'> } = $props();
      ...
      <div class="space-y-5">
        ...
        {:else if state.activeStage === 'evidence'}
          <WorkspaceSplitPane secondaryWidth="38%">
            ...
            {#snippet secondary()}<SectionHeader title={isZh ? '证据预览' : 'Evidence preview'} description={isZh ? '查看、标注和补齐证据。' : 'Review, annotate, and complete evidence.'} /><div class="mt-4 space-y-3"><MediaThumbnail src={null} emptyLabel={isZh ? '尚未上传 X-Ray 切片' : 'No X-Ray slice uploaded'} showOverlay={false} /><div class="flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground"><span>X-Ray slice 02</span><span>{isZh ? '待补齐 · 1024 × 768 · 4×' : 'Pending · 1024 × 768 · 4×'}</span></div></div>{/snippet}
          </WorkspaceSplitPane>
        {:else}
