<script lang="ts">
  ...
  import { valueText, valueNumber, countBy, sumBy, averageBy, findLabel } from '../../rendering-values';
  ...
  import { captureAdminContext, useList, useUpdateMany } from '@svadmin/core';
  ...
  type DomainResource = ResourcesForFeature<'domain'> | 'products';
  ...
  let { resourceName }: { resourceName: DomainResource } = $props();
  ...
  const rawRows = $derived(currentQuery.data?.data ?? []);
  const products = $derived(demoRenderers.products.records(productsQuery.data?.data ?? []));
  const categories = $derived(demoRenderers.categories.records(categoriesQuery.data?.data ?? []));
  ...
  const findName = <T extends { id: number }>(records: readonly T[], id: unknown, key?: keyof T) =>
    findLabel(records, id, isZh ? '未分配' : 'Unassigned', key);
  ...
  const summaryMetricBuilders: Record<DomainResource, SummaryMetricBuilder> = {
    ...
    skus: () => {
      ...
      [isZh ? '生效' : 'Active', countBy(rows, 'status', 'active')],
    ...
    categories: () => {
      const rows = demoRenderers.categories.records(rawRows);
      return [
    ...
    suppliers: () => {
      const rows = demoRenderers.suppliers.records(rawRows);
      return [
    ...
    warehouses: () => {
      const rows = demoRenderers.warehouses.records(rawRows);
      return [
      [isZh ? '仓库' : 'Warehouses', rows.length],
      [isZh ? '总容量' : 'Capacity', sumBy(rows, 'capacity')],
      [isZh ? '平均利用率' : 'Avg utilization', `${averageBy(rows, 'utilization')}%`],
    ...
    notifications: () => {
      const rows = demoRenderers.notifications.records(rawRows);
      return [
      [isZh ? '通知' : 'Notifications', rows.length],
      [isZh ? '未读' : 'Unread', rows.filter((row) => !row['read']).length],
      [isZh ? '严重' : 'Critical', countBy(rows, 'severity', 'critical')],
      [isZh ? '渠道' : 'Channels', new Set(rows.map((row) => row['channel'])).size],
      ];
    },
    store_client_products: () => {
      const rows = demoRenderers.store_client_products.records(rawRows);
      return [
      [isZh ? '在售商品' : 'Store products', rows.length],
      [isZh ? '平均评分' : 'Avg rating', rows.length ? (sumBy(rows, 'rating') / rows.length).toFixed(1) : '0'],
      [isZh ? '可售库存' : 'Available stock', sumBy(rows, 'stock')],
      [isZh ? '商品价值' : 'Merchandise value', money(sumBy(rows, 'price'))],
      ];
    },
    store_client_orders: () => {
      const rows = demoRenderers.store_client_orders.records(rawRows);
      return [
      [isZh ? '商城订单' : 'Store orders', rows.length],
      [isZh ? '交易金额' : 'Order value', money(sumBy(rows, 'totalAmount'))],
      [isZh ? '处理中' : 'Processing', countBy(rows, 'status', 'processing')],
      [isZh ? '已签收' : 'Delivered', countBy(rows, 'status', 'delivered')],
      ];
    },
    project_planning: () => {
      ...
      [isZh ? '进行中' : 'In progress', countBy(rows, 'status', 'in_progress')],
      [isZh ? '待评审' : 'In review', countBy(rows, 'status', 'review')],
      [isZh ? '平均信心度' : 'Avg confidence', `${averageBy(rows, 'confidence')}%`],
      ];
    },
    store_admin: () => {
      ...
      [isZh ? '进行中' : 'In progress', countBy(rows, 'status', 'in_progress')],
      [isZh ? '待复核' : 'In review', countBy(rows, 'status', 'review')],
      [isZh ? '负责人' : 'Owners', new Set(rows.map((row) => row['ownerId'])).size],
      ];
    },
    store_services: () => {
      const rows = demoRenderers.store_services.records(rawRows);
      return [
      [isZh ? '服务' : 'Services', rows.length],
      [isZh ? '运行环境' : 'Runtimes', new Set(rows.map((row) => row['runtime'])).size],
      [isZh ? '平均延迟预算' : 'Avg latency budget', `${averageBy(rows, 'latencyBudgetMs')} ms`],
      [isZh ? '进行中' : 'In progress', countBy(rows, 'status', 'in_progress')],
      ];
    },
    ai_prompt: () => {
      const rows = demoRenderers.ai_prompt.records(rawRows);
      return [
      [isZh ? '提示词' : 'Prompts', rows.length],
      [isZh ? '使用次数' : 'Uses', sumBy(rows, 'usageCount')],
      [isZh ? '待复核' : 'In review', countBy(rows, 'status', 'review')],
    ...
    billing_plans: () => {
      const rows = demoRenderers.billing_plans.records(rawRows);
      return [
      [isZh ? '套餐' : 'Plans', rows.length],
      [isZh ? '生效套餐' : 'Active plans', countBy(rows, 'status', 'active')],
      ...
      [isZh ? '包含席位' : 'Included seats', sumBy(rows, 'seatsIncluded')],
      ];
    },
    billing_invoices: () => {
      const rows = demoRenderers.billing_invoices.records(rawRows);
      return [
      [isZh ? '账单' : 'Invoices', rows.length],
      [isZh ? '账单金额' : 'Billed', money(sumBy(rows, 'amount'))],
      [isZh ? '待收款' : 'Pending', money(rows.filter((row) => valueText(row, 'status') === 'pending').reduce((sum, row) => sum + valueNumber(row, 'amount'), 0))],
      [isZh ? '失败账单' : 'Failed', countBy(rows, 'status', 'failed')],
      ];
    },
    billing_subscriptions: () => {
      const rows = demoRenderers.billing_subscriptions.records(rawRows);
      return [
      [isZh ? '订阅' : 'Subscriptions', rows.length],
      [isZh ? '活跃订阅' : 'Active', countBy(rows, 'status', 'active')],
      [isZh ? '试用' : 'Trials', countBy(rows, 'status', 'trialing')],
      [isZh ? '逾期' : 'Past due', countBy(rows, 'status', 'past_due')],
      ];
    },
    security_sessions: () => {
      const rows = demoRenderers.security_sessions.records(rawRows);
      return [
      [isZh ? '会话' : 'Sessions', rows.length],
      [isZh ? '活跃' : 'Active', countBy(rows, 'status', 'active')],
      [isZh ? '空闲' : 'Idle', countBy(rows, 'status', 'idle')],
      [isZh ? '已撤销' : 'Revoked', countBy(rows, 'status', 'revoked')],
      ];
    },
    security_devices: () => {
      const rows = demoRenderers.security_devices.records(rawRows);
      return [
      [isZh ? '设备' : 'Devices', rows.length],
      [isZh ? '可信设备' : 'Trusted', countBy(rows, 'status', 'trusted')],
      [isZh ? '待审批' : 'Pending', countBy(rows, 'status', 'pending')],
      [isZh ? '平台数' : 'Platforms', new Set(rows.map((row) => row['platform'])).size],
      ];
    },
    security_allowed_ips: () => {
      const rows = demoRenderers.security_allowed_ips.records(rawRows);
      return [
      [isZh ? 'IP 规则' : 'IP rules', rows.length],
      [isZh ? '允许' : 'Allowed', countBy(rows, 'status', 'allowed')],
      [isZh ? '受限' : 'Restricted', countBy(rows, 'status', 'restricted')],
    ...
    referral_invites: () => {
      ...
      [isZh ? '已接受' : 'Accepted', countBy(rows, 'status', 'accepted')],
      [isZh ? '待接受' : 'Open', countBy(rows, 'status', 'sent')],
      [isZh ? '转化率' : 'Conversion', `${percent(countBy(rows, 'status', 'accepted'), rows.length)}%`],
      ];
    },
  };

  const summaryMetrics = $derived(summaryMetricBuilders[domainResource]());
  ...
  function requestSessionRevoke(target: DemoRow<'security_sessions'> | 'others'): void {
  ...
  async function confirmSessionRevoke(): Promise<void> {
    ...
    if (!target || resourceName !== 'security_sessions') return;
    const rows = demoRenderers.security_sessions.records(rawRows);
    const sessions = target === 'others'
      ? rows.filter((row) => !row['current'] && row['status'] !== 'revoked')
      : [demoRenderers.security_sessions.record(target)];
    const ids = sessions.map(row => {
      if (typeof row['id'] !== 'number' || !Number.isFinite(row['id'])) throw new Error('Invalid session ID');
      return row['id'];
    });
