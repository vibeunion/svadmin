...
<div data-app-page="case-workspace" data-resource-name={resourceName} data-active-stage={state.activeStage}>
  <ContentPageShell pageId="case-workspace" width="wide" class="space-y-5">
    ...
    {#snippet contextContent()}
      <WorkspaceInspector title={isZh ? '案件上下文' : 'Case context'} description={isZh ? '不离开当前阶段查看关联信息。' : 'Related information without leaving the stage.'} open={state.inspectorOpen} ontoggle={(open) => state.toggleInspector(open)}>
        <div class="space-y-5">
