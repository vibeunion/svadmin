<script lang="ts">
  ...
  import { actorLabel, permissionLabel, stateLabel, actionLabel, recordLabel, auditLabel } from './zh-CN';
  import './office.css';
  ...
  const filteredReports = $derived(reports.filter((item) => (statusFilter === 'all' || item.status === statusFilter) && `${recordLabel(item.id)} ${item.person} ${item.institution}`.toLowerCase().includes(search.trim().toLowerCase())));
...
<ContentPageShell pageId="health-office" width="wide">
  <div class="office" data-office-workspace data-office-view={view}>
    ...
    <header class="office-header">
      <div><p class="office-eyebrow">报告审核工作平台</p><h1>智能辅助办公</h1><p class="office-subtitle">从报告接入到审核闭环，让每一项提示都有明确的下一步。</p></div>
    ...
    <div class="office-context"><span>{activeView.page.replace('UI-', '页面 ')} / {activeView.label}</span><span>北区演示范围 · 不含其他部门记录</span></div>
    ...
    {:else if view === 'dashboard'}
      ...
      <div class="office-grid">
        <section class="office-panel"><div class="office-panel-heading"><SectionHeader title="优先处理" description="一级预警优先，其次完成数据核对。" /><Button size="sm" variant="outline" onclick={() => navigate('alerts')}>全部预警</Button></div>
          {#each alerts.filter((item) => item.state !== 'closed').sort((a, b) => a.level - b.level) as item (item.id)}<div class="office-task"><div class="office-task-number" class:critical={item.level === 1}>{item.level}</div><div class="office-task-text"><strong>{item.title}</strong><p>{recordLabel(item.reportId)} · {alertLabels[item.state]} · 责任人：{actorLabel(item.owner)}</p><small>演示到期时间：{formatTime(item.dueAt)}（上海时区）</small></div><Button size="sm" onclick={() => navigate('alert', item.reportId, item.id)}>处理任务</Button></div>{/each}
          {#if metrics.open === 0}<div class="office-empty"><p>当前范围内没有未关闭预警。</p></div>{/if}
        </section>
        <aside class="office-panel"><SectionHeader title="审核完成条件" description="缺失的检查不会被系统当作正常。" /><div class="office-checklist"><p><span>01</span>身份和关键指标已核对</p><p><span>02</span>异常检查与文本校对有结果</p><p><span>03</span>预警处置及必要复核完成</p><p><span>04</span>无待办建议、未保存修改或冲突</p></div><Button variant="outline" onclick={() => navigate('reports')}>进入报告中心</Button><div class="office-note">真实文字识别、校对、预警服务与通知网关尚未接入。失败样例提供人工接管入口。</div></aside>
      </div>
      <section class="office-panel"><div class="office-panel-heading"><SectionHeader title="继续核对报告" description="所有人员、指标和结果均为合成演示数据。" /><Button size="sm" onclick={() => navigate('imports')}>导入预检</Button></div><div class="office-table-wrap"><table><caption class="office-sr-only">需要核对的报告</caption><thead><tr><th>报告与人员</th><th>机构</th><th>质量状态</th><th>下一步</th></tr></thead><tbody>{#each reports.filter((item) => item.quality !== 'verified') as item (item.id)}<tr><td><strong>{item.person}</strong><small>{recordLabel(item.id)}</small></td><td>{item.institution}</td><td>{item.quality === 'blocked' ? '身份冲突，禁止直接通过' : '待人工核对'}</td><td><Button size="sm" variant="outline" onclick={() => navigate('verification', item.id)}>查看并核对</Button></td></tr>{/each}</tbody></table></div></section>
    {:else if view === 'reports'}
      <section class="office-panel"><div class="office-panel-heading"><SectionHeader title="报告中心" description="保留原始值、来源、当前检查状态与审核版本。" /><Button size="sm" onclick={() => navigate('imports')}>导入报告</Button></div><div class="office-toolbar"><label class="office-grow">搜索授权范围<input aria-label="搜索报告" placeholder="报告编号、合成人员或机构" bind:value={search} oninput={() => { reportPage = 1; }} /></label><label>审核状态<select bind:value={statusFilter} onchange={() => { reportPage = 1; }}><option value="all">全部状态</option><option value="pending">待审核</option><option value="in_review">审核中</option><option value="complete">已完成</option></select></label><Button variant="outline" size="sm" onclick={() => { search = ''; statusFilter = 'all'; reportPage = 1; }}>重置条件</Button></div>
        <div class="office-table-wrap"><table><caption class="office-sr-only">授权范围内报告列表</caption><thead><tr><th>报告 / 人员</th><th>机构 / 日期</th><th>质量状态</th><th>检查状态</th><th>审核状态</th><th>操作</th></tr></thead><tbody>{#each pageRows as item (item.id)}<tr><td><strong>{item.person}</strong><small>{recordLabel(item.id)} · {recordLabel(item.maskedIdentity)}</small></td><td>{item.institution}<small>{item.date}</small></td><td>{item.quality === 'verified' ? '已核对' : item.quality === 'blocked' ? '身份冲突' : '待核对'}</td><td>{checkLabels[item.anomaly]}</td><td><StatusBadge status={stateTone(item.status)} label={reportLabels[item.status]} /></td><td><Button size="sm" variant="outline" onclick={() => navigate('report', item.id)}>查看报告</Button></td></tr>{/each}</tbody></table></div>
    ...
    {:else if view === 'imports'}
      <section class="office-panel"><SectionHeader title="批量导入预检" description="先校验来源与文件，再由接入服务创建解析任务。" /><div class="office-note">本页仅检查文件名、大小和扩展名，不读取内容、不上传、不进行病毒检测或真实格式判定。有效文件显示“等待接入”，不是“解析成功”。</div><form novalidate class="office-import" onsubmit={(event) => { event.preventDefault(); if (run({ type: 'import', files: selectedFiles }, '预检清单已建立。所有文件仍保留在本机，未创建报告。')) selectedFiles = []; }}><label>来源机构<select disabled><option>演示机构·北区（固定样例）</option></select></label><label>选择文件（建议每批 100 个、单文件 50 兆字节）<input type="file" bind:this={fileInput} hidden multiple accept=".pdf,.png,.jpg,.jpeg,.xlsx,.docx" aria-label="选择报告文件" onchange={(event) => { selectedFiles = Array.from(event.currentTarget.files ?? []).map((file) => ({ name: file.name, size: file.size })); }} /></label><Button type="button" variant="outline" onclick={() => fileInput?.click()}>选择报告文件</Button><p>已选择 {selectedFiles.length} 个文件。请勿选择真实健康资料。</p><Button type="submit" disabled={!selectedFiles.length}>建立本地预检清单</Button></form></section>
      <section class="office-panel"><SectionHeader title="导入项" description="逐项呈现错误，不伪造处理百分比。" /><div class="office-table-wrap"><table><thead><tr><th>文件</th><th>状态</th><th>原因</th><th>操作</th></tr></thead><tbody>{#each data.imports as item (item.id)}<tr><td>{item.filename}<small>{recordLabel(item.id)} · {item.size} 字节</small></td><td>{item.state === 'blocked' ? '等待接入' : item.state === 'rejected' ? '已拒绝' : '已取消'}</td><td>{actionLabel(item.reason)}</td><td><Button variant="outline" size="sm" disabled={item.state !== 'blocked'} onclick={() => run({ type: 'cancel-import', jobId: item.id }, '已取消该未上传项，既有报告未被删除。')}>取消</Button></td></tr>{/each}</tbody></table></div>{#if !data.imports.length}<div class="office-empty"><p>尚无导入项。文件预检失败不会影响其他项。</p></div>{/if}</section>
    ...
    {:else if view === 'alert'}
      {#if !alert}<section class="office-empty"><h2>预警不存在或不在授权范围内</h2><Button onclick={() => navigate('alerts')}>返回预警中心</Button></section>{:else}
        <section class="office-report-header"><div><p class="office-eyebrow">{recordLabel(alert.id)} / 第 {alert.revision} 版</p><h2>{alert.title}</h2><p>{recordLabel(alert.reportId)} · 责任人：{actorLabel(alert.owner)}</p></div><StatusBadge status={stateTone(alert.state)} label={alertLabels[alert.state]} /></section><div class="office-grid"><section class="office-panel"><SectionHeader title="处置操作" description="先确认接手，再填写结论与依据。" /><div class="office-note">消息已读不改变此状态。误报和转诊必须提供核实或交接证据；一级预警必须由不同人员复核。</div>
          ...
          {:else if alert.state === 'processing'}<form novalidate onsubmit={(event) => { event.preventDefault(); run({ type: 'propose', alertId: alert.id, revision: alertRevision, result: disposition, note, evidence }, '处置已记录；一级预警或误报、转诊结论进入独立复核。'); }}><label>处置结论<select aria-label="处置结论" bind:value={disposition}><option value="handled">已处理</option><option value="false_positive">核实为误报</option><option value="referral">已完成交接</option></select></label><label>处理意见<textarea aria-label="处理意见" bind:value={note} required minlength="4"></textarea></label><label>核实依据 / 接收方交接确认<textarea aria-label="交接证据" bind:value={evidence} required={disposition !== 'handled'} placeholder="演示凭证编号或交接说明；转诊通知本身不代表完成交接。"></textarea></label><Button type="submit" disabled={alert.owner !== actor.id}>提交处置</Button></form>
          {:else if alert.state === 'review'}<div class="office-note">待独立复核。提交人：{actorLabel(alert.proposedBy)}。请切换为不同的演示业务复核员。</div><label>复核意见<textarea aria-label="复核意见" bind:value={note}></textarea></label><Button disabled={!can(actor, 'review') || alert.proposedBy === actor.id} onclick={() => run({ type: 'approve', alertId: alert.id, revision: alertRevision, note }, '独立复核已通过，预警已关闭并保留证据。')}>复核通过并关闭</Button>
        ...
        </section><aside class="office-panel"><SectionHeader title="依据与处理记录" description="仅呈现输入结果，不生成医学结论。" /><dl class="office-facts"><div><dt>结果来源</dt><dd>固定合成数据／第一版样例</dd></div><div><dt>演示到期时间</dt><dd>{formatTime(alert.dueAt)} · 上海时区</dd></div><div><dt>处置意见</dt><dd>{alert.note || '尚未提交'}</dd></div><div><dt>交接 / 核实证据</dt><dd>{alert.evidence || '尚未提供'}</dd></div></dl><div class="office-timeline">{#each data.changes.filter((item) => item.kind === 'alert' && item.object === alert.id) as item (item.id)}<article><strong>{actorLabel(item.actor)} · {formatTime(item.time)}</strong><p>{actionLabel(item.reason)}</p>{#if 'state' in item.after}<small>状态：{stateLabel(item.after.state)}</small>{/if}</article>{/each}</div><Button variant="outline" onclick={() => navigate('alerts')}>返回预警中心</Button></aside></div>
      {/if}
    {:else if view === 'analytics'}
      <section class="office-metrics"><MetricBlock label="报告快照" value={String(metrics.reports)} detail="不是时间段处理量" /><MetricBlock label="已完成报告" value={String(metrics.complete)} detail="按当前有效报告计数" /><MetricBlock label="演示观测异常率" value={metrics.rate} detail={`${metrics.abnormal} / ${metrics.denominator} 个可判定观测`} /><MetricBlock label="排除观测" value={String(metrics.excluded)} detail="缺失、未判定、失败或过期" /></section><section class="office-panel"><SectionHeader title="口径与可追溯明细" description="当前展示授权范围的合成快照，不伪造历史趋势或工作效率提升。" /><div class="office-note">异常率分母只含检查成功或有人工记录、且结果可判定的有效观测；无法判定单独展示。固定样例中的异常率不具有医学意义。</div><div class="office-table-wrap"><table><thead><tr><th>报告</th><th>检查状态</th><th>审核状态</th><th>查看明细</th></tr></thead><tbody>{#each reports as item (item.id)}<tr><td>{recordLabel(item.id)}</td><td>{checkLabels[item.anomaly]}</td><td>{reportLabels[item.status]}</td><td><Button size="sm" variant="outline" onclick={() => navigate('report', item.id)}>钻取报告</Button></td></tr>{/each}</tbody></table></div><div class="office-actions"><Button variant="outline" disabled>电子表格与文档导出（未接入）</Button><Button variant="outline" disabled>定时报表（未接入）</Button></div><p class="office-muted">生产版本需冻结统计窗口、时区、去重口径和受控下载权限；当前不提供公开下载地址。</p></section>
    {:else if view === 'rules'}
      <section class="office-panel"><SectionHeader title="规则审批流程演示" description="不配置生产医学阈值，不执行组合判定。" /><div class="office-note">规则内容编辑器、医学完整性校验、试运行、回滚与真实发布服务尚未接入。以下仅演示职责分离和状态迁移；演示发布不会生效或改写历史报告。</div><label>提交 / 复核说明<textarea bind:value={note} aria-label="规则处理说明" placeholder="至少 4 个字符。"></textarea></label>{#each data.rules as item (item.id)}<article class="office-rule"><div><strong>{item.title}</strong><p>{recordLabel(item.id)} · 流程版本 {item.version} · {item.scope}</p><small>编辑者：{actorLabel(item.author)} / 复核者：{item.reviewer ? actorLabel(item.reviewer) : '尚未复核'}</small><p>{item.note}</p></div><div class="office-stack"><StatusBadge status={stateTone(item.state)} label={ruleLabels[item.state]} />{#if item.state === 'draft'}<Button size="sm" disabled={item.author !== actor.id} onclick={() => run({ type: 'rule', ruleId: item.id, version: item.version, action: 'submit', note }, '演示草稿已提交，需不同人员复核。')}>提交草稿</Button>{:else if item.state === 'review'}<Button size="sm" disabled={!can(actor, 'publish') || item.author === actor.id} onclick={() => run({ type: 'rule', ruleId: item.id, version: item.version, action: 'approve', note }, '独立复核流程已通过；未运行医学规则。')}>独立复核</Button>{:else if item.state === 'approved'}<Button size="sm" disabled={!can(actor, 'publish')} onclick={() => run({ type: 'rule', ruleId: item.id, version: item.version, action: 'publish', note }, '演示发布状态已记录；没有真实规则生效。')}>演示发布状态</Button>{/if}</div></article>{/each}</section><section class="office-panel"><SectionHeader title="白名单申请" description="忽略单条建议不产生全局白名单。" />{#each data.dictionary as item (item.id)}<div class="office-task"><div class="office-task-text"><strong>{item.term}</strong><p>{item.scope} · 提交人：{actorLabel(item.requester)}</p></div><StatusBadge status="warning" label="申请中 · 未生效" /></div>{/each}{#if !data.dictionary.length}<div class="office-empty"><p>暂无申请。工作人员可以从校对页发起。</p></div>{/if}<p class="office-muted">词库新增、查询、修改与删除、白名单审批与词典版本分发需真实后端；本轮未将其标记为已完成。</p></section>
    {:else if view === 'messages'}
      <section class="office-panel"><SectionHeader title="站内消息" description="已读状态按演示身份记录，与业务任务确认分离。" />{#each messages as item (item.id)}<article class="office-task"><div class="office-task-text"><strong>{item.title}</strong><p>{recordLabel(item.id)} · {recordLabel(item.reportId)}</p><small>{item.readBy.includes(actor.id) ? '当前身份已读' : '当前身份未读'}</small></div><div class="office-actions"><Button size="sm" variant="outline" disabled={item.readBy.includes(actor.id)} onclick={() => run({ type: 'read-message', messageId: item.id }, '消息已读，关联预警仍需单独确认和处置。')}>标为已读</Button><Button size="sm" onclick={() => navigate('report', item.reportId)}>打开报告</Button></div></article>{/each}{#if !messages.length}<div class="office-empty"><p>当前授权范围没有消息。</p></div>{/if}<div class="office-note">邮件、短信与超时通知未接入。站内演示不代表通知已送达。</div></section>
    {:else if view === 'users'}
      <section class="office-panel"><SectionHeader title="用户与任务移交" description="停用人员前处理未完成任务，不删除其历史记录。" /><div class="office-note">本页仅演示两名业务人员的停用与任务移交，不修改 svadmin 登录账号。用户创建、角色授权及服务器会话失效仍需后端实施。</div><label class="office-checkbox"><input type="checkbox" bind:checked={transfer} />确认停用时将未完成报告、预警转移至原部门队列</label><div class="office-table-wrap"><table><thead><tr><th>演示账号</th><th>状态</th><th>操作</th></tr></thead><tbody>{#each data.staff as item (item.id)}<tr><td>{item.name}<small>{recordLabel(item.id)}</small></td><td>{item.active ? '启用' : '已停用'}</td><td><Button variant="outline" size="sm" disabled={!item.active} onclick={() => { if (window.confirm(`停用 ${item.name}？未完成任务须按部门移交，历史记录保留。`)) run({ type: 'disable-user', userId: item.id, transfer }, '演示账号已停用，未完成任务已移交，历史记录保留。'); }}>停用演示账号</Button></td></tr>{/each}</tbody></table></div></section><section class="office-panel"><SectionHeader title="职责分离" description="页面授权只是交互示例，不是安全边界。" /><div class="office-table-wrap"><table><thead><tr><th>身份</th><th>能力包</th><th>数据范围</th></tr></thead><tbody>{#each actors as item (item.id)}<tr><td>{item.name}</td><td>{item.permissions.map(permissionLabel).join('、')}</td><td>{item.departments.map((department) => department === 'north' ? '北区' : department === 'south' ? '南区' : '未识别范围').join('、')}</td></tr>{/each}</tbody></table></div></section>
    {:else if view === 'audit'}
      <section class="office-panel"><SectionHeader title="本会话操作记录" description="只读展示；不是持久化、防篡改的生产审计系统。" /><div class="office-note">运行记录不包含完整报告正文，业务修改快照在报告历史中查看。刷新会清空本会话；生产须由后端追加记录并实施保留策略。</div><div class="office-table-wrap"><table><thead><tr><th>时间（上海）</th><th>操作人</th><th>动作</th><th>对象</th></tr></thead><tbody>{#each data.audit.filter((item) => actor.departments.includes(item.department)).reverse() as item (item.id)}<tr><td>{formatTime(item.time)}</td><td>{actorLabel(item.actor)}</td><td>{auditLabel(item.action)}</td><td>{recordLabel(item.object)}</td></tr>{/each}</tbody></table></div></section>
    {:else if view === 'integrations'}
      <section class="office-panel"><SectionHeader title="能力与接入状态" description="接口预留不等于集成完成。没有服务时不提供虚假的成功按钮。" /><div class="office-capabilities">{#each [{ title: '报告接入与文字识别', detail: '仅本地文件元数据预检。实际上传、扫描、结构化和原文定位未接入。' }, { title: '异常判定 / 文本校对', detail: '固定合成结果与人工接管流程。没有医学判断或纠错算法。' }, { title: '医保、医院信息系统与医疗数据标准', detail: '来源清单、接口版本、授权和字段映射待确认；未进行真实联调。' }, { title: '通知与受控导出', detail: '邮件、短信、定时报表、审批下载与撤销尚未接入。' }] as capability (capability.title)}<article><StatusBadge status="neutral" label="未接入" /><h3>{capability.title}</h3><p>{capability.detail}</p></article>{/each}</div><div class="office-note">疾病预测为二期预留，当前关闭；不会上传模型、训练或调用公网智能服务。</div></section><section class="office-panel"><SectionHeader title="后端命令接入约束" description="为联调保留必要的身份、版本与失败边界。" /><dl class="office-facts"><div><dt>请求上下文</dt><dd>服务端会话、数据范围、对象标识、预期版本、幂等标识、请求追踪标识。</dd></div><div><dt>冲突与失败</dt><dd>403 无权访问；409 版本冲突；422 业务门禁；503 能力不可用。失败不返回“正常”。</dd></div><div><dt>异步回调</dt><dd>校验来源和报告 / 文本版本；重复回调只应用一次；过期结果不覆盖新结果。</dd></div><div><dt>上线门槛</dt><dd>真实授权、持久化、审计、通知、导出与恢复测试；不能以本页演示替代。</dd></div></dl></section>
    {/if}
    <footer class="office-footer"><span>智能辅助办公 · 需求说明第 1.1 版 · 页面与流程验证</span><span>不提供自动诊断 · 不处理真实健康资料</span></footer>
  </div>
</ContentPageShell>
