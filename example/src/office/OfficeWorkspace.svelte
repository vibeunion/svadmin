<script lang="ts">
  ...
  import { beforeEach } from '@svadmin/core';
  ...
  import './office.css';
  import { exampleHistoryIndex, registerExampleHistoryGuard } from '../history-index';
  ...
  let hostLeaveApproved = false;
  let historyIndex = 0;
  let restoringHistory = false;

  function currentHistoryIndex(): number {
    const index = exampleHistoryIndex();
    if (index === undefined) throw new Error('历史记录跟踪未初始化，请刷新工作区');
    return index;
  }
  ...
  const dirty = $derived(contentDirty || (view === 'alert' && disposition !== 'handled') || (['verification', 'proofreading', 'alert', 'rules'].includes(view) && (note.trim().length > 0 || evidence.trim().length > 0)));
  ...
  function navigate(next: OfficeView, id?: string, taskId?: string): void {
    ...
    acceptedLocation = url.href;
    historyIndex = currentHistoryIndex();
  }
  function readLocation(): void {
    acceptedLocation = window.location.href;
    historyIndex = currentHistoryIndex();
  ...
  function verifyReport(event: SubmitEvent): void {
    event.preventDefault();
    if (!report) return;
    const changed = workingValue !== report.observations[0]?.value;
    run({ type: 'verify', reportId: report.id, revision: formRevision, note, observationId: 'obs-a', value: workingValue },
      changed ? '核对记录已保存；变更的指标需重新检查，关联预警已重新打开，待核实。' : '核对记录已保存；未变更指标，原有检查和预警状态保持不变。');
  }
  onMount(() => {
    readLocation();
    const removeGuard = beforeEach(() => {
      hostLeaveApproved = confirmLeave();
      return hostLeaveApproved;
    });
    ...
    const popState = (event: PopStateEvent) => {
      ...
      // 在宿主路由处理前拦截浏览器历史跳转，避免先卸载再恢复而丢失草稿。
      if (!hostLeaveApproved && dirty && !window.confirm('返回将放弃未保存修改，是否继续？')) {
        event.stopImmediatePropagation();
        const delta = historyIndex - currentHistoryIndex();
        if (delta !== 0) {
      ...
      hostLeaveApproved = false;
      if (window.location.hash.split('?')[0] === '#/health_office') readLocation();
    };
    const hashChange = (event: HashChangeEvent) => {
      if (restoringHistory || window.location.href === acceptedLocation && dirty) event.stopImmediatePropagation();
    };
    window.addEventListener('beforeunload', beforeUnload);
    const removeHistoryGuard = registerExampleHistoryGuard({ popstate: popState, hashchange: hashChange });
    return () => { removeGuard(); removeHistoryGuard(); window.removeEventListener('beforeunload', beforeUnload); };
  });
</script>
...
<ContentPageShell pageId="health-office" width="wide">
  <div class="office" data-office-workspace data-office-view={view}>
    ...
    {:else if view === 'dashboard'}
      ...
      <div class="office-grid">
        <section class="office-panel"><div class="office-panel-heading"><SectionHeader title="优先处理" description="一级预警优先，其次完成数据核对。" /><Button size="sm" variant="outline" onclick={() => navigate('alerts')}>全部预警</Button></div>
          {#each alerts.filter((item) => item.state !== 'closed').sort((a, b) => a.level - b.level || a.dueAt.localeCompare(b.dueAt)) as item (item.id)}<div class="office-task"><div class="office-task-number" class:critical={item.level === 1}>{item.level}</div><div class="office-task-text"><strong>{item.title}</strong><p>{item.reportId} · {alertLabels[item.state]} · 责任人：{item.owner === 'queue' ? '部门队列' : item.owner}</p><small>演示到期时间：{formatTime(item.dueAt)}（上海时区）</small></div><Button size="sm" onclick={() => navigate('alert', item.reportId, item.id)}>处理任务</Button></div>{/each}
    ...
    {:else if view === 'report' || view === 'verification' || view === 'proofreading'}
      ...
      {:else}
        ...
        {#if view === 'report'}
          ...
          </section><aside class="office-panel">
            <SectionHeader title="完成审核检查" description="任何阻塞未解除，都不能标记完成。" />
            <dl class="office-facts"><div><dt>异常检查</dt><dd>{checkLabels[report.anomaly]}</dd></div><div><dt>文本校对</dt><dd>{checkLabels[report.proof]}</dd></div></dl>
            {#each blockers as item (item)}<p class="office-blocker">待完成 · {item}</p>{/each}
            {#if blockers.length}
              <div class="office-stack">
                {#if report.quality !== 'verified' || !['success', 'manual'].includes(report.anomaly)}<Button variant="outline" size="sm" onclick={() => navigate('verification', report.id)}>核对数据与异常检查</Button>{/if}
                {#if !['success', 'manual'].includes(report.proof) || suggestions.some(item => item.state === 'pending')}<Button variant="outline" size="sm" onclick={() => navigate('proofreading', report.id)}>处理校对与建议</Button>{/if}
                {#each alerts.filter(item => item.reportId === report.id && item.state !== 'closed') as task (task.id)}<Button variant="outline" size="sm" onclick={() => navigate('alert', report.id, task.id)}>处理预警 {task.id}</Button>{/each}
              </div>
            {:else}<p class="office-note">完成条件已满足，可以提交审核结果。</p>{/if}
            <Button disabled={blockers.length > 0 || report.status === 'complete'} onclick={() => run({ type: 'complete', reportId: report.id, revision: formRevision }, '本会话报告审核已完成，后续修改将重开。')}>完成审核</Button><p class="office-muted">PDF / Excel 导出尚未接入受控导出服务。</p><Button size="sm" variant="outline" disabled>受控导出（未接入）</Button>
          </aside></div>
        ...
        {:else if view === 'verification'}
          <div class="office-grid">
            <section class="office-panel">
              <SectionHeader title="数据核对" description="修正派生值，不覆盖原始结果。" />
              {#if report.quality === 'blocked'}<div class="office-note office-danger">身份匹配有冲突。需要来源系统核实和受控改绑，本演示不会提供一键绕过。</div>{/if}
              <dl class="office-facts"><div><dt>原始项目结果</dt><dd>{report.observations[0]?.original}</dd></div><div><dt>源单位</dt><dd>{report.observations[0]?.unit}</dd></div><div><dt>来源</dt><dd>固定合成文本；原始文件未接入</dd></div></dl>
              <form onsubmit={verifyReport}>
                <label>检测项目 A 当前结果<input bind:value={workingValue} aria-label="核对指标结果" required /></label>
                <label>核对依据与修改原因<textarea bind:value={note} minlength="4" required placeholder="至少 4 个字符，说明核对范围与依据；请勿填写真实健康信息。"></textarea></label>
                <div class="office-actions"><Button type="submit" disabled={report.quality === 'blocked'}>保存并确认核对</Button><Button type="button" variant="outline" onclick={loadForms}>放弃未保存修改</Button>{#if dirty}<span class="office-muted">尚未保存</span>{/if}</div>
              </form>
            </section>
            <aside class="office-panel"><SectionHeader title="检查失败时人工接管" description="分别记录人工异常检查与人工校对，不自动判定正常。" /><p>{checkLabels[report.anomaly]} / {checkLabels[report.proof]}</p><p class="office-muted">使用左侧填写的依据。必须先完成数据核对；校对仍有待办建议时不能跳过。</p><div class="office-stack"><Button variant="outline" disabled={contentDirty} onclick={() => manual('anomaly')}>记录人工异常检查完成</Button><Button variant="outline" disabled={contentDirty} onclick={() => manual('proof')}>记录人工校对完成</Button></div></aside>
          </div>
        {:else}
    ...
    {:else if view === 'alert'}
      {#if !alert}<section class="office-empty"><h2>预警不存在或不在授权范围内</h2><Button onclick={() => navigate('alerts')}>返回预警中心</Button></section>{:else}
        <section class="office-report-header"><div><p class="office-eyebrow">{alert.id} / V{alert.revision}</p><h2>{alert.title}</h2><p>{alert.reportId} · 责任人：{alert.owner === 'queue' ? '部门队列' : alert.owner}</p></div><StatusBadge status={stateTone(alert.state)} label={alertLabels[alert.state]} /></section><div class="office-grid"><section class="office-panel"><SectionHeader title="处置操作" description="先确认接手，再填写结论与依据。" /><div class="office-note">消息已读不改变此状态。误报和转诊必须提供核实或交接证据；一级预警必须由不同人员复核。</div>
          ...
          {:else if alert.state === 'review'}<div class="office-note">待独立复核。提交人：{alert.proposedBy}。请切换为不同的演示业务复核员。</div><label>复核意见<textarea aria-label="复核意见" bind:value={note}></textarea></label>
            <div class="office-actions"><Button disabled={!can(actor, 'review') || alert.proposedBy === actor.id} onclick={() => run({ type: 'approve', alertId: alert.id, revision: alertRevision, note }, '独立复核已通过，预警已关闭并保留证据。')}>复核通过并关闭</Button>
              <Button variant="outline" disabled={!can(actor, 'review') || alert.proposedBy === actor.id} onclick={() => run({ type: 'reject', alertId: alert.id, revision: alertRevision, note }, '已退回原负责人补充处置，预警仍未关闭。')}>退回补充处置</Button></div>
    ...
    {:else if view === 'audit'}
      <section class="office-panel"><SectionHeader title="本会话操作记录" description="只读展示；不是持久化、防篡改的生产审计系统。" /><div class="office-note">运行记录不包含完整报告正文，业务修改快照在报告历史中查看。刷新会清空本会话；生产须由后端追加记录并实施保留策略。</div><div class="office-table-wrap"><table><thead><tr><th>时间（上海）</th><th>操作人</th><th>动作</th><th>对象</th></tr></thead><tbody>{#each data.audit.filter((item) => actor.departments.includes(item.department)).reverse() as item (item.id)}<tr><td>{formatTime(item.time)}</td><td>{item.actor}</td><td>{item.action}</td><td>{item.object}</td></tr>{:else}<tr><td colspan="4">当前授权范围尚无本会话操作记录。</td></tr>{/each}</tbody></table></div></section>
