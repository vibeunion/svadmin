# Admin UI browser specimens

这是现有设计任务的可运行浏览器样例，不是新 UI 框架，不是 Figma 图层导入器。
它消费真实 `packages/ui/dist` 中的 Svelte 组件及 `app.css`，不复制第三方图层、品牌或字体，不另加样式框架，不修改生产组件，不注册 Surface 权限。
...
```sh
node --test design/admin-ui/preview/model.test.mjs
node node_modules/svelte-check/bin/svelte-check --tsconfig design/admin-ui/preview/tsconfig.json --fail-on-warnings
bunx playwright install --with-deps chromium
node design/admin-ui/preview/verify.mjs
node design/admin-ui/preview/keyboard.mjs
```

专项工作流输出 `test-results/admin-ui-browser/`：可运行静态站点 `site/`、截图浏览 `index.html`、逐场景 `report.json`、键盘与媒体偏好结果 `keyboard.json`、截图及源文件哈希。解压 artifact 后，可用 `python3 -m http.server 4179 --directory site` 预览，无需为消费端安装 Panda/Tailwind。不要直接用 file:// 打开 ES 模块入口。
