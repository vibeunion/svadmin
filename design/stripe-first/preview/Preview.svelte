<script lang="ts">
  ...
  const filtered = $derived(filterCustomers(query));
  let sample = $state('');
...
<div class:dark={theme === 'dark'} class="svadmin-theme kit-root" data-theme={theme} data-preview-density={density}>
  ...
  <div class="kit-layout">
    <nav class="kit-nav" aria-label={tx('设计样例', 'Design examples')}>
      <p class="kit-eyebrow">DESIGN REFERENCE</p>
      {#each Object.keys(scenarios) as key (key)}
        <button type="button" data-testid={'nav-' + key} aria-current={view === key ? 'page' : undefined} onclick={() => navigate(key)}>{viewLabel(key)}</button>
      {/each}
      ...
      <main id="kit-main" tabindex="-1" data-testid="specimen" data-view={view} data-scenario={scenario}>
          ...
          {:else}
            ...
            {:else}
              ...
              <div class="kit-table-scroll" role="region" aria-label={tx('客户数据', 'Customer data')} tabindex="0"><Table.Root {density}>
                ...
                <Table.Body>{#each filtered as customer (customer.id)}<Table.Row><Table.Cell><span class="kit-customer-name">{customer.name}</span><span class="kit-cell-sub">{customer.email}</span></Table.Cell><Table.Cell><StatusBadge status={customer.status === 'active' ? 'success' : 'warning'} label={customer.status === 'active' ? tx('已启用', 'Active') : tx('待审核', 'Pending')} /></Table.Cell><Table.Cell class="kit-number">{money(customer.amount)}</Table.Cell><Table.Cell><Button data-testid={'open-' + customer.id} variant="outline" size="sm" onclick={() => { selectedId = customer.id; void navigate('record-detail'); }}>{tx('查看', 'View')}</Button></Table.Cell></Table.Row>{/each}</Table.Body>
              </Table.Root></div>
              <p class="kit-muted" data-testid="count">{filtered.length} {tx('位客户', 'customers')}</p>
            {/if}
          {/if}
        {:else if view === 'record-detail'}
          ...
          {:else if selected}
            ...
            <section class="kit-section" aria-labelledby="attributes-title"><h2 id="attributes-title">{tx('客户信息', 'Customer information')}</h2><DescriptionList columns={2} {density} items={[{ label: tx('邮箱', 'Email'), value: selected.email }, { label: tx('创建日期', 'Created'), value: selected.created }, { label: tx('交易总额', 'Volume'), value: money(selected.amount) }, { label: tx('数据来源', 'Data source'), value: tx('合成演示数据', 'Synthetic fixture') }]} /></section>
            <section class="kit-section" aria-labelledby="activity-title"><h2 id="activity-title">{tx('活动记录', 'Activity')}</h2>
